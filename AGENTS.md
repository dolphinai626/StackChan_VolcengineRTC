...
- **同步/推送 main 前必须核对历史完整性**：`git pull --ff-only` 干净不代表安全。
  本项目发生过历史重写（rebase/force-push）丢掉已验证修复的事故（commit 80296ad 的
  idle 续命修复被丢，提醒 TTS 注入后会话永不退出待机，详见 0ef2a6b）。做 main 同步时
  除了看工作树干净，还要 `git log --oneline` / `git reflog` 对比近期 main 历史，
  确认 debug-*.md 与 WORKLOG.md 记录的已验证 commit 仍在线，发现缺失先停下问用户
