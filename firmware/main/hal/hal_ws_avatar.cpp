        ...
        if (msg.binary) {
            ...
            switch (type) {
                ...
                case DataType::Jpeg: {
                    ...
                    if (getPayload(msg, payload, payload_len)) {
                        ESP_LOGI(_tag.c_str(), "Jpeg Frame Received, size: %d", (int)payload_len);
                        volc_agent::sendVideoJpeg(payload, payload_len);

                        static int64_t _time_count = 0;
                        static int64_t _interval   = 0;
                        _time_count                = esp_timer_get_time();

                        size_t jpeg_len    = payload_len;
                        uint8_t* jpeg_data = (uint8_t*)heap_caps_malloc(jpeg_len, MALLOC_CAP_8BIT);
                        if (jpeg_data) {
                            memcpy(jpeg_data, payload, jpeg_len);
...
private:
    ...
    bool getPayload(const ReceivedMessage& msg, const uint8_t*& payload, size_t& payload_len)
    {
        if (msg.data.size() < 5) {
            return false;
        }
        ...
        uint32_t net_len = 0;
        memcpy(&net_len, msg.data.data() + 1, sizeof(net_len));
        payload_len = ntohl(net_len);
        if (payload_len != msg.data.size() - 5) {
            ESP_LOGE(_tag.c_str(), "Invalid packet length: header=%d, actual=%d", (int)payload_len,
                     (int)(msg.data.size() - 5));
            return false;
        }

        payload = msg.data.data() + 5;
        return true;
    }
