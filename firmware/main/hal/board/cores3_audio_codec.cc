...
void CoreS3AudioCodec::EnableInput(bool enable) {
    ...
    if (enable) {
        ...
        auto mic_gain_mask = ESP_CODEC_DEV_MAKE_CHANNEL_MASK(0) | ESP_CODEC_DEV_MAKE_CHANNEL_MASK(1);
        ESP_ERROR_CHECK(esp_codec_dev_set_in_channel_gain(input_dev_, mic_gain_mask, input_gain_));
    } else {
        ESP_ERROR_CHECK(esp_codec_dev_close(input_dev_));
    }
