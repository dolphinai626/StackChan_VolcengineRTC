...
bool Hal::updateFirmware(std::function<void(std::string_view)> onLog)
{
    ...
    mclog::tagWarn(_tag, "new firmware available: version={}, device OTA disabled", firmware_version);
    if (!firmware_version.empty()) {
        onLog(std::string("New firmware found: ") + firmware_version + " (manual upgrade required)");
    } else {
        onLog("New firmware found (manual upgrade required)");
    }
    return true;
}
