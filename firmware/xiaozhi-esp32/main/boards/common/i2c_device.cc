...
void I2cDevice::WriteReg(uint8_t reg, uint8_t value) {
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "WriteReg(0x%02x)=0x%02x failed: %s", reg, value, esp_err_to_name(err));
    }
}
...
void I2cDevice::ReadRegs(uint8_t reg, uint8_t* buffer, size_t length) {
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "ReadRegs(0x%02x, len=%u) failed: %s", reg, (unsigned)length, esp_err_to_name(err));
    }
}
