...
bool AfeWakeWord::Initialize(AudioCodec* codec, srmodel_list_t* models_list) {
    ...
    // The audio detection task stack must live in PSRAM: by the time wake word
    // detection is enabled (entering idle after activation) internal SRAM is
    // heavily fragmented and a normal xTaskCreate would fail to allocate the
    // stack, leaving nobody to fetch() from AFE (ringbuffer full, no wake-up).
    const size_t stack_size = 4096;
    if (audio_detection_task_stack_ == nullptr) {
        audio_detection_task_stack_ = (StackType_t*)heap_caps_malloc(stack_size, MALLOC_CAP_SPIRAM);
